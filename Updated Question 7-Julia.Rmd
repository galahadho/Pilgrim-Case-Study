...
``To create a regression first comparing profitabilty and online channel (original found in question 6) that controlled for omitted variable biases we first had to randomize the data and create training and validation sets. For the first regression comparing profit and online usage, each variable added (tenure, age, income, district, and billpay) increased adjusted R-squared and had significance, therefore they were important omitted variables to consider in our regression.  
```{r}
